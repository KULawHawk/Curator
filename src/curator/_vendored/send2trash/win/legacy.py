"""Windows ctypes-based send2trash implementation.

Vendored from send2trash/win/legacy.py (BSD-3-Clause), Copyright 2017
Virgil Dupras.

Uses the SHFileOperationW Win32 API to send files to the Recycle Bin.
This is the older API (the modern alternative is IFileOperation, which
requires pywin32 — we skip it for Phase Alpha to keep dependencies
minimal).

Curator-local modifications: only the import path
``from send2trash.util import preprocess_paths`` becomes
...
def convert_sh_file_opt_result(result):
    """Map SHFileOperationW return codes to standard Windows errors.

    SHFileOpterationW has its own error codes that overlap with standard
    Win32 errors. This translation makes the messages more legible and
    consistent with what users see elsewhere.

    Refs:
      * https://docs.microsoft.com/en-us/windows/win32/api/shellapi/nf-shellapi-shfileoperationw#return-value
...
def prefix_and_path(path):
    ...
    Local paths (``C:\folder\file.ext``) and UNC names
    (``\\server\folder\file.ext``) are handled.

    Returns a tuple of ``(prefix, prefixed_path)``.
    """
...
def get_awaited_path_from_prefix(prefix, path):
    """Reverse :func:`prefix_and_path` so the result can go to SHFileOperationW.

    The long-path prefix must be removed; UNC names need a backslash put back.
    """
    if prefix == "\\\\?\\UNC":
...
def get_short_path_name(long_name):
    """Resolve to the 8.3-ish short path name (used to bypass MAX_PATH)."""
...
def send2trash(paths):
    """Move ``paths`` (str | iterable of str) to the Recycle Bin."""
    paths = preprocess_paths(paths)
    if not paths:
        return
    # convert data type
    paths = [
        str(path, "mbcs") if not isinstance(path, str) else path
        for path in paths
    ]
    ...
    #     LPCWSTR(path + '\0') directly
    # because embedded null characters are no longer allowed in strings.
    # Workaround:
    ...
    # Additional note: unicode_buffer expects length in bytes essentially,
    # so multi-byte characters cause issues if just passing python's string
    # length. Instead of dealing with this difference we just create a buffer
    # then a new one with an extra null. Since the non-length-specified
    # version apparently stops after the first null, join with a space first.
