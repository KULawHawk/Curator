...
**Status:** v0.3 — IMPLEMENTED 2026-05-08. The full 3-session plan from §5 has shipped end-to-end. P1 (Curator-side `curator_plugin_init` hookspec) released as Curator v1.1.2 (commit cd357fd, tag v1.1.2). P2 (`curatorplug-atrium-safety` v0.2.0 consuming the hookspec for independent re-read verification) released as plugin v0.2.0 (commit e28473c, tag v0.2.0). P3 (this stamp + regression sweep + safety-plugin DESIGN.md v0.3 stamps) closes 2026-05-08. Total tests added: 24 (6 in Curator + 18 in plugin); regression remained green throughout (348/348 Curator slice, 71/71 plugin suite).
**Date:** 2026-05-08
...
- **2026-05-08 v0.2** — RATIFIED. Jake ratified all 4 DM recommendations (DM-1 through DM-4) as written without modification. Doc status flips from "design proposal" to "approved spec"; P1 implementation cleared to begin. No structural changes to the design — only ratification-status flips on each DM and the v0.2 revision-log entry.
- **2026-05-08 v0.3** — IMPLEMENTED. All three sessions of §5's plan have shipped:
  * **P1 — Curator v1.1.2 (~30 min planned, ~30 min actual).** Commit cd357fd, tag v1.1.2 pushed to GitHub. `curator_plugin_init(pm)` hookspec added under a new "Plugin lifecycle (v1.1.2+)" section in `src/curator/plugins/hookspecs.py`. Wiring in `_create_plugin_manager` fires it as the LAST step (per DM-2) inside a defensive try/except (per DM-3). 6 new unit tests in `tests/unit/test_plugin_manager.py` regression-guard each ratified DM. Curator slice 342/342 → 348/348; plugin's existing 53 tests still pass with the new hook firing on each test runtime startup.
  * **P2 — plugin v0.2.0 (~1.0h planned, ~1.0h actual).** Commit e28473c, tag v0.2.0 pushed to GitHub. `AtriumSafetyPlugin.curator_plugin_init` hookimpl saves `self.pm`. New two-phase enforcement in `curator_source_write_post`: Phase 1 = existing `decide()` logic (refusal on skipped verify); Phase 2 = new `_verify_via_re_read` (chunked re-read via `pm.hook.curator_source_read_bytes`, hash compare, refuse on strict-mode mismatch / warn on lax-mode mismatch / OK on conservative fallback). 14 new unit tests + 4 new integration tests; total plugin suite 53 → 71. Headline test `test_re_read_catches_post_write_corruption_in_strict` proves end-to-end that re-read catches what single-shot verify misses (a `tryfirst=True` hookimpl corrupts dst between verify and re-read; the safety plugin reads via LocalPlugin, sees the corruption, raises `ComplianceError`, MigrationService converts to `MigrationOutcome.FAILED`).
  * **P3 — regression sweep + doc stamps (~30 min planned, ~30 min actual).** This commit. `curatorplug-atrium-safety/DESIGN.md` v0.2 → v0.3 IMPLEMENTED with the full implementation history under §9. This doc v0.2 → v0.3 IMPLEMENTED with the v0.3 revision-log entry you are reading. Final regression: 348/348 Curator slice + 71/71 plugin suite + 0 failures, no behavior change for users not running plugins that consume the hookspec.
  * **Backward compat invariants from §6 verified:** existing core plugins (`local`, `gdrive`) and pre-`curator_plugin_init` plugins (atrium-safety v0.1.0) continue to work; the only behavior change visible to users is the new defense-in-depth refusal cases in atrium-safety v0.2.0 strict mode, which is the intended headline value.
  * **What's next (out of scope for this design):** the four DMs about hookspec signature/timing/failure-handling/dynamic-registration are answered. Future related work will be its own design+ratification cycle. Specifically: a `curator_audit_event` hookspec (so plugins can write structured audit entries via Curator's AuditRepository instead of just logging to `loguru`) is the natural next addition — atrium-safety's DM-3 anticipates it but it is NOT covered by this design.
