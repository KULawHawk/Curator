...
**Status:** v0.3 — IMPLEMENTED 2026-05-08. P1 (retry decorator + integration) shipped at commit `fe5739f`; P2 (conflict resolution + 4 modes + 3 new MigrationOutcome variants + MigrationConflictError) shipped at commit `08db2de`; P3 (this revision — README + CHANGELOG + version bump + tag) ships as Curator v1.3.0. Curator regression slice 414 → 444 (+30 tests across both sessions). Plugin suite (atrium-safety v0.3.0) 75/75 unchanged. See §12 revision log for the v0.3 entry with both commit hashes, test deltas, and lessons captured. Earlier state preserved: v0.2 RATIFIED 2026-05-08 (Jake ratified all 6 DMs as recommended); v0.1 DRAFT 2026-05-08 (skeleton with 6 DM recommendations).
**Date:** 2026-05-08
...
- **2026-05-08 v0.3** — IMPLEMENTED. All three sessions shipped against the v0.2 RATIFIED plan. Final test count delta exceeded the v0.2 estimate by 5 (predicted +25, actual +30). Curator regression slice: **414 → 444 (+30)** across the migration + plugin + audit + MCP + integration + GUI test suites. Plugin suite (atrium-safety v0.3.0): 75/75 unchanged. No schema changes. No version bumps for atrium-safety or any plugin. Doc expanded from ~600 lines (v0.2) to ~720 lines (v0.3) with this revision entry.

  **P1 — retry decorator + integration. Commit `fe5739f`.** Shipped per §5.1 plan: new module `src/curator/services/migration_retry.py` (148 LOC) with `_is_retryable` helper + `retry_transient_errors` stateless decorator reading `self._max_retries` / `self._retry_backoff_cap` / `self.audit`. `migration.py` modifications: `_max_retries=3` and `_retry_backoff_cap=60.0` instance attrs in `__init__`; `set_max_retries(n)` method clamping to `[0, 10]`; `@retry_transient_errors` applied to `_cross_source_transfer` at line 970; `apply()` and `run_job()` now actually call `self.set_max_retries(max_retries)` (CRITICAL FIX — the `max_retries` parameter was previously accepted but ignored, a silent gap discovered during code-touchpoint verification at v0.1 issuance). `cli/main.py`: `--max-retries N` Typer option (default 3, capped 10); wired through Phase 1 (apply call) and Phase 2 (options dict + run_job call) paths. Tests: `tests/unit/test_migration_phase3_retry.py` (~280 LOC, 15 tests — not 10 as the v0.2 estimate predicted): `TestIsRetryable` (6) + `TestRetryDecorator` (7) + `TestServiceIntegration` (2). Test fixture issues caught and fixed during implementation: `_FakeResp` needed `.reason` attribute for `googleapiclient.errors.HttpError`; `socket.timeout` is just an alias for `TimeoutError` in Python 3.10+. Two deliberate simplifications from design §4.4 documented in module docstring: (1) only-cross-source decoration (not `_execute_one_same_source` — same-source local-FS errors are mostly permanent and don't benefit from retry), (2) stateless decorator instead of the lazy-callable form proposed in §4.4 (the stateless form's introspection of `self._max_retries` at decorate-time is cleaner). Curator regression slice: **414 → 429 (+15)**. Plugin suite 75/75 unchanged.

  **P2 — conflict resolution. Commit `08db2de`.** Shipped per §5.2 plan: 3 new `MigrationOutcome` values (`MOVED_OVERWROTE_WITH_BACKUP`, `MOVED_RENAMED_WITH_SUFFIX`, `FAILED_DUE_TO_CONFLICT`) + `MigrationConflictError` exception class (carries `dst_path` + `src_path`). `MigrationReport.moved_count` / `failed_count` / `bytes_moved` updated via `ClassVar` tuples (`_MOVED_VARIANTS`, `_FAILED_VARIANTS`) so the user-facing tallies span all variants. `MigrationService` additions: `_on_conflict_mode` instance attr (default `'skip'`), `_VALID_CONFLICT_MODES` ClassVar frozenset, `set_on_conflict_mode` method validating against the frozenset (raises `ValueError` so the CLI surfaces a clean error), helpers `_compute_backup_path` (`<name>.curator-backup-<UTC-iso8601-compact><ext>`), `_find_available_suffix` (lowest `n` in `[1, 9999]` for `<name>.curator-<n><ext>`; raises `RuntimeError` after 9999), `_audit_conflict`, `_resolve_collision` (for `MigrationMove`), `_resolve_collision_for_progress` (for `MigrationProgress` — sister method instead of awkward polymorphism, since progress doesn't carry an in-memory outcome field). Dispatch sites wired: `apply()` Gate 3 mode dispatch + outcome_override; `apply()` and `run_job()` accept new `on_conflict` parameter (default `'skip'`); `run_job()` reads `on_conflict` from `job.options` for resumed jobs (mirrors `max_retries` pattern); `_execute_one_persistent_same_source` collision dispatch with outcome_override on MOVED return; `_execute_one_cross_source` and `_execute_one_persistent_cross_source` SKIPPED_COLLISION dispatch for full skip+fail support; `_worker_loop` catches `MigrationConflictError` specifically (mapped to `FAILED_DUE_TO_CONFLICT`, distinct from `FAILED`) and recognizes the new MOVED variants in the success branch. CLI: `--on-conflict MODE` Typer option (`skip`|`fail`|`overwrite-with-backup`|`rename-with-suffix`; default `skip`). Phase 1 path catches `MigrationConflictError` and renders a clean error with src + dst paths; exit code 1. Phase 2 path includes the mode in the options dict + passes to `run_job`. Tests: `tests/unit/test_migration_phase3_conflict.py` (~310 LOC, 15 tests): `TestSkipMode` (1) + `TestFailMode` (2) + `TestOverwriteWithBackup` (3) + `TestRenameWithSuffix` (3) + `TestAuditConflictDetails` (3) + `TestServiceClamping` (3). **Cross-source simplification documented:** `overwrite-with-backup` and `rename-with-suffix` DEGRADE TO SKIP with a warning + audit when src + dst differ (the plugin contract lacks an atomic-rename hook, so a clean cross-source implementation isn't yet possible). Documented in commit message + module docstring + audit details (`mode: "<mode>-degraded-cross-source"` with `cross_source: True` + `reason: "plugin contract lacks atomic-rename hook"`). **Bug caught during own self-review of the diff:** Phase 1 cross-source raise was happening BEFORE `report.moves.append(move)` — fixed by moving the raise from `_execute_one_cross_source` into `apply()`'s post-`_execute_one` block, mirroring same-source pattern; otherwise the failed move would have been lost from the report. Curator regression slice: **429 → 444 (+15)**.

  **P3 — release ceremony. Curator v1.3.0.** This commit. Updates: README status line v1.2.0 → v1.3.0; README documentation list adds v1.3.0 to release history + adds `TRACER_PHASE_3_DESIGN.md` v0.3 IMPLEMENTED entry; README "Migration (Tracer)" section gets a Phase 3 subsection (~50 lines) documenting `--max-retries` + `--on-conflict` with examples per mode; this design doc stamped v0.2 → v0.3 IMPLEMENTED with this revision-log entry; CHANGELOG `## [1.3.0]` entry added (~70 lines); version bumped 1.2.0 → 1.3.0 in `src/curator/__init__.py` + `pyproject.toml`. Tag `v1.3.0` pushed.

  **Lessons captured (now 6-for-6 read-code-first applications across atrium-* / PLUGIN_INIT / CURATOR_AUDIT_EVENT / MCP / Tracer Phase 3):** (a) reading actual code at design-doc-issuance time caught the silent retry-claim gap from Phase 2 (where the design said "retry once on transient error" but no retry code existed) before designing retry logic around it; (b) at P1 issuance, all code-touchpoint claims in §4.4 were verified by direct reading; (c) at P1 implementation, two deviations from §4.4 (only-cross-source decoration + stateless decorator vs. lazy-callable) were caught BEFORE coding and documented in the module docstring so the deviations weren't silent; (d) at P1 implementation, the silent CLI flag bug (`apply()` accepted `max_retries` but never used it) was caught and fixed; (e) at P2 issuance, 6 of 6 method-signature mismatches between the design pseudocode and actual code (`MigrationProgress` vs `MigrationMove` field divergence, etc.) were caught BEFORE coding and resolved by writing a sister `_resolve_collision_for_progress` method instead of forcing awkward polymorphism; (f) at P2 implementation, the raise-before-append bug in Phase 1 cross-source was caught DURING self-review of the diff and fixed before commit. The 6-for-6 tally is:
  ...
  **Phase 4+ deferrals stand** (§11 unchanged): proactive bandwidth throttling, per-source retry policy, `curator migrate-cleanup-backups` utility, retry observability dashboards, async retry refactor, and — new from P2 — cross-source `overwrite-with-backup` + `rename-with-suffix` (requires expanding the source-plugin hookspec to add an atomic-rename hook or exists-probe hook). All explicit non-scope for v1.3.0.

  Doc state: Phase 3 closed. Next Tracer revision is Phase 4 if and when the deferred items become high-priority again; the audit channel + retry/conflict surface established here is the foundation for any future enhancement.
