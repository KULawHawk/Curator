...
**Status:** v0.1 — DRAFT 2026-05-08. Skeleton only. §1 scope identifies what Phase 4 ships against actual code (verified at issuance — see §1.1 footnote on `curator_source_move`'s gdrive stub). §3 lists 6 DMs awaiting Jake's ratification with recommendations. Implementation is **NOT cleared at v0.1**; awaits Jake's `1` reply against §3 wishlist (same `ratify`-default convention as all prior atrium-* / PLUGIN_INIT / CURATOR_AUDIT_EVENT / MCP_SERVER / Tracer Phase 3 plans). Ratification → v0.2 RATIFIED → P1 lands as the first commit of the v1.4.0 cycle.
**Date:** 2026-05-08
