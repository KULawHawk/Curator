...
**Status:** v0.2 — RATIFIED 2026-05-08. Jake ratified all 6 DMs as recommended (`continue` reply against the v0.1 DRAFT wishlist; per operating rule 12, default `continue` after a design doc = ratify all DMs as recommended). Implementation cleared to begin. P1 lands the new `curator_source_rename` hookspec + impls in local + gdrive + ~10 tests. P2 wires the cross-source dispatch update. P3 ships as Curator v1.4.0 (per DM-6) with README + design v0.3 IMPLEMENTED stamp + CHANGELOG + tag. See §5 for the implementation plan and §12 revision log for the v0.2 entry. Earlier state preserved: v0.1 DRAFT 2026-05-08 (skeleton with 6 DM recommendations, code-grounded findings re: `curator_source_move`'s gdrive stub).
**Date:** 2026-05-08
