...
|---|---|---|---|
| **Curator** | **v1.6.1 stable** ✅ (released 2026-05-09) | `C:\Users\jmlee\Desktop\AL\Curator\` (HEAD on GitHub, tag `v1.6.1`) | Content-aware artifact intelligence layer. SQLite index + plugin framework + lineage tracking + Tracer migration engine + MCP server (with Bearer-auth HTTP) + PySide6 GUI. v1.6.1 made `migration.move` and `migration.copy` audit details schema-symmetric across all four code paths (`cross_source` / `src_source_id` / `dst_source_id` now always present); unblocks atrium-citation v0.2 cross-source filtering. v1.6.0 added `curator sources config` subcommand closing the v1.5.0 CLI gap that helper-script setup_gdrive_source.py worked around (18 new integration tests; 41/41 sources CLI tests pass). v1.5.1 fixed two architectural bugs in the gdrive plugin's SourceConfig resolution + parent_id translation; cross-source local→Drive migration end-to-end validated 2026-05-09 (Session B: 10 files migrated, 0 failures, 18.59s real Drive round-trip; files verified in Drive folder `1hIavkBD...` with correct sizes / timestamps / content / owner). v1.5.0 added MCP HTTP-auth (BearerAuthMiddleware + `curator mcp keys` CLI + audit emission). v1.4.1 added sentinel-default API hardening for migration policy kwargs. |
