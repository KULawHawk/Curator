...
|---|---|---|---|
| **Curator** | **v1.6.2 stable** ✅ (released 2026-05-09) | `C:\Users\jmlee\Desktop\AL\Curator\` (HEAD on GitHub, tag `v1.6.2`) | Content-aware artifact intelligence layer. SQLite index + plugin framework + lineage tracking + Tracer migration engine + MCP server (with Bearer-auth HTTP) + PySide6 GUI. **v1.6.2** added GUI Tools menu (5 placeholder dialogs for v1.7) + Workflows menu (5 launchers for click-to-run batch workflows) + bulletproof installer with real-MCP-probe verification gate (Step 9). Five PowerShell batch workflows shipped at `Curator/scripts/workflows/`: 01_initial_scan, 02_find_duplicates, 03_cleanup_junk, 04_audit_summary, 05_health_check — each with cautious safety rails (plan-mode preview, explicit user confirmation, recycle-bin reversibility). Comprehensive `docs/USER_GUIDE.md` (700 lines) covers every CLI subcommand, MCP tool, GUI tab, configuration option, and includes 8 copy-pasteable workflow recipes. Full GUI v2 design captured at `docs/design/GUI_V2_DESIGN.md` (v1.7 foundation parity → v1.8 advanced → v1.9 native workflows + atrium-reversibility integration). v1.6.1 made `migration.move` and `migration.copy` audit details schema-symmetric across all four code paths (`cross_source` / `src_source_id` / `dst_source_id` now always present); unblocks atrium-citation v0.2 cross-source filtering. v1.6.0 added `curator sources config` subcommand. v1.5.1 fixed two architectural bugs in the gdrive plugin's SourceConfig resolution + parent_id translation; cross-source local→Drive migration end-to-end validated 2026-05-09. v1.5.0 added MCP HTTP-auth (BearerAuthMiddleware + `curator mcp keys` CLI + audit emission). v1.4.1 added sentinel-default API hardening for migration policy kwargs. |
