...
|---|---|---|---|
| **Curator** | **v1.5.1 stable** ✅ (released 2026-05-09; cross-source production-validated) | `C:\Users\jmlee\Desktop\AL\Curator\` (HEAD on GitHub, tag `v1.5.1`) | Content-aware artifact intelligence layer. SQLite index + plugin framework + lineage tracking + Tracer migration engine + MCP server (with Bearer-auth HTTP) + PySide6 GUI. v1.5.1 fixed two architectural bugs in the gdrive plugin's SourceConfig resolution + parent_id translation; cross-source local→Drive migration end-to-end validated 2026-05-09 (Session B: 10 files migrated, 0 failures, 18.59s real Drive round-trip; files verified in Drive folder `1hIavkBD...` with correct sizes / timestamps / content / owner). v1.5.0 added MCP HTTP-auth (BearerAuthMiddleware + `curator mcp keys` CLI + audit emission). v1.4.1 added sentinel-default API hardening for migration policy kwargs. |
