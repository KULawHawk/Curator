...
|---|---|---|---|
| **Curator** | **v1.7.148 mature** ✅ — approaching v2.0-RC (released 2026-05-13) | `C:\Users\jmlee\Desktop\AL\Curator\` (HEAD on GitHub, tag `v1.7.148`) | Content-aware artifact intelligence layer. SQLite index + plugin framework + lineage tracking + Tracer migration engine + MCP server (with Bearer-auth HTTP) + PySide6 GUI. **148 versioned releases. 55 modules at 100% line + branch coverage under the apex-accuracy doctrine. 5 multi-ship engineering arcs closed in May 2026.** The arcs (May 7 → May 13): Migration Phase Gamma (v1.7.89–93b, 19 services modules), Coverage Sweep (v1.7.95–106, 12 sweep targets), Plugins+MCP+Config Sweep (v1.7.116–125, 9 modules), Storage Repositories Sweep (v1.7.126–137, 11 modules — entire `storage/` subpackage), Mid-Size Services Sweep (v1.7.140–145, 5 services). Round 3 in progress (v1.7.146+): stabilization sprint + CLI Coverage Arc push toward v2.0. **What it does:** Curator gives every file a stable identity, tracks relationships and lineage with confidence scores, knows where files belong, and makes every destructive operation reversible. **Major v1.6+ features (still current):** GUI Tools menu + Workflows menu, bulletproof installer with real-MCP-probe gate, 5 PowerShell batch workflows (01_initial_scan, 02_find_duplicates, 03_cleanup_junk, 04_audit_summary, 05_health_check), comprehensive `docs/USER_GUIDE.md`, GUI v2 design at `docs/design/GUI_V2_DESIGN.md`, audit details schema-symmetric across all four code paths (unblocks atrium-citation v0.2 cross-source filtering), `curator sources config` subcommand, gdrive plugin SourceConfig resolution fix, MCP HTTP-auth (BearerAuthMiddleware + `curator mcp keys` CLI), sentinel-default API hardening for migration policy kwargs. **Doctrine artifacts (engineering-arc legacy):** [`CLAUDE.md`](Curator/CLAUDE.md) carries the apex-accuracy doctrine, lesson adoption protocol, and 95 numbered lessons documenting patterns that compounded across the arc. Three `# pragma: no cover/branch` annotations in the codebase, all with documented Lesson #91 rationale. |
...
| **curatorplug-atrium-citation** | **v0.2.0** ✅ (released 2026-05-09) | `C:\Users\jmlee\Desktop\AL\curatorplug-atrium-citation\` (HEAD on GitHub, tag `v0.2.0`, repo `https://github.com/KULawHawk/curatorplug-atrium-citation`) | Constitutional plugin for Atrium **Principle 3 (Citation Chain Preservation)** + **Aim 5 (Fidelity)** at v0.3 ordering. v0.2 implements the cross-source filter that DM-2 RATIFIED specified but v0.1 couldn't deliver until Curator v1.6.1 made audit details schema-symmetric. Sweep now filters to genuine cross-source events by default; v0.1-style "flag everything" behavior available via `curator-citation sweep --all-events`. 5 new DMs ratified for v0.2 (DM-7 through DM-11). 47/47 unit tests passing (35 from v0.1 + 12 new v0.2). |
