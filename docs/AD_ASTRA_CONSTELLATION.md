...
|---|---|---|---|
| **Curator** | **v1.5.0 stable** ✅ (released 2026-05-08) | `C:\Users\jmlee\Desktop\AL\Curator\` (HEAD pending, tag `v1.5.0`) | Content-aware artifact intelligence layer. SQLite index + plugin framework + lineage tracking + Tracer migration engine + MCP server (now with Bearer-token HTTP authentication) + PySide6 GUI. The most complete pillar by lines of code and test coverage. v1.5.0 added MCP HTTP-auth (BearerAuthMiddleware + `curator mcp keys` CLI + audit emission) closing the v1.5.0 candidate item per Tracer Phase 4 v0.2 RATIFIED DM-6. v1.4.1 added sentinel-default API hardening for migration policy kwargs. |
